...
| [`prompt-caching.md`](prompt-caching.md) | the loop note's "cheaper first turn" — does Gemini cache the 1,410-token prefix the agent re-sends every turn? | Implicitly never (under the minimum); explicitly yes, −40% gross, −28% net of storage at 5 q/h against a 30% rule. No knob; `cached_tokens` reported and priced |
| [`floor-fallback.md`](floor-fallback.md) | after sub-floor ordering — if a search that clears nothing returns the dense order, marked, can the agent use it? | Not for the answerable questions it was built for (the dense order picks a title chunk over the answer paragraph; q48 is a 0.0129-vs-0.01 coin toss either way); but 15/15 unanswerable still decline and the loop stops flailing — searches 2.9 → 1.2, cost halved. Knob kept, default off; efficiency claim queued with its own rule |
| [`floor-fallback-cost.md`](floor-fallback-cost.md) | the queued cost claim — on all 55 answerable questions, does the fallback make the model decline where control would have answered? | It does not (false abstention 1 → 0, faithfulness up), but the mechanism fired on **1 of 55**: the pre-registered exposure clause failed, every number that moved is loop nondeterminism, and the candidate is closed |
...
6. ~~IVF with an HNSW coarse quantizer at 1M vectors: the point where
   ...
   stack instead of compete.~~ Done — [`ivf-hnsw-1m.md`](ivf-hnsw-1m.md):
   at 65k cells the graph finds 99% of brute's cells and cuts the coarse
   step 3–5×, but the coarse step is under half the query, so the whole
   query drops 1.6–1.8×, not the 2× claimed; and 65k cells at 1M costs
   0.05 recall against `4√n`. The trick belongs to 10M+.
